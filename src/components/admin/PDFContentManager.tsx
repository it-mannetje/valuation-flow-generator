  ...
  const handleImageUpload = async (type: 'background' | 'middle_image' | 'image1' | 'image2' | 'logo', file: File) => {
    try {
      ...
      toast({
        ...
        description: `${type === 'background' ? 'Achtergrond' : 'Afbeelding'} geconverteerd naar base64 en geüpload.`,
      });
    } catch (error) {
    ...
    try {
      ...
      if (latestRequest) {
        ...
        companyData = {
          ...
          result2024: latestRequest.result_previous_year || 200000,
          expectedResult2025: latestRequest.result_current_year || 250000,
          wasLossmaking: false,
